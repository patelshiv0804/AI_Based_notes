...
import { Brain, Sparkles, Tag, FileText, Languages, X, Loader } from 'lucide-react';
...
function AIPanel({ note, onUpdateNote }) {
	...
	const generateAISuggestions = async () => {
		...
		try {
			...
			setGlossary(Array.isArray(groqResult.glossary) ? groqResult.glossary : []);
	...
	const translateNote = async () => {
		...
		try {
			...
			const translatedContent = await AIService.translateNote(plainContent, selectedLanguage);
								...
								{Array.isArray(glossary) && glossary.length > 0 ? (
									<>
										<div className="glossary-items custom-scrollbar">
											{glossary.slice(0, showAllGlossary ? undefined : 2).map((item, index) => (
												<div key={`${item.term}-${index}`} className="glossary-item">
										...
										{glossary.length > 2 && (
											...
											>
												{showAllGlossary ? 'Show Less' : `Show ${glossary.length - 2} More Terms`}
											</button>
										)}
									...
									<p className="placeholder">AI will identify and define important terms from your note...</p>
								)}
							</div>
						</div>
						...
						<div className="ai-section" style={{overflowX: 'hidden'}}>
							...
							<div className="grammar-content">
								{Array.isArray(grammar) ? (
									grammar.length > 0 ? (
										<div className="grammar-issues">
											<div className="grammar-status error">
												Found {grammar.length} grammar issue{grammar.length !== 1 ? 's' : ''}:
											</div>
									...
									<p className="placeholder">Click "Generate Summary" to check grammar...</p>
								)}
							</div>
						</div>
							...
							<div className="insights-content">
								<div className="insight-item">
									...
									<span className="insight-value">
										{Math.ceil((note.content.replace(/<[^>]*>/g, '').length / 200))} min
									</span>
								</div>
								<div className="insight-item">
